...
if let roomCount = edgar.residence?.numberOfRooms{
    print("La casa tiene \(roomCount) habitación(es)")
}else{
    print("No tienes casa")
}
...
func createAddress() -> Address{
    ...
    someAddress.street = "Calle de las sirenas"
...
if let edgarStreet = edgar.residence?.address?.street{
    print("La casa esta en la calle \(edgarStreet)")
}else{
...
if let beginsWith13 = edgar.residence?.address?.buildingIdentifier()?.hasPrefix("13"){
    print("vives en el número 13")
}else{
    print("No se donde vives")
}
